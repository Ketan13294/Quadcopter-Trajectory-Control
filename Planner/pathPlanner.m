...
function waypts = pathPlanner(start,goal,plt)
    ...
    % %% Optimize path
    % inflate(map2D,0.1);
    % inpath = pthObj.States(:,1:3);
    % options = optimizePathOptions;
    % options.MinTurningRadius = 1;                   % Minimum turning radius (same as in the planner)
    % options.MaxPathStates = size(inpath,1)*3;     % Maximum number of poses allowed in the optimized path
    % options.ObstacleSafetyMargin = 1.0;             % Safe distance of 2.0 meters from the obstacles.
    % optpath = optimizePath(inpath,map2D,options);   % Optimize the path generated by the planner.

    % % waypts = minjerkpolytraj(pthObj.States(:,1:3)',[0:length(pthObj.States)-1],100,TimeAllocation=true)';
    % waypts = minsnappolytraj(optpath(:,1:3)',[0:length(optpath)-1],1000,TimeAllocation=true)';
    ...
    if plt ==1
        ...
        hold on
        % Tree points
        plot3(solnInfo.TreeData(:,1),solnInfo.TreeData(:,2), solnInfo.TreeData(:,3),'b-'); % tree expansion
        % Raw Path
        plot3(pthObj.States(:,1),pthObj.States(:,2),pthObj.States(:,3),'r-','LineWidth',1) % draw path
        % Optimal Path
        % plot3(optpath(1:end,1),optpath(1:end,2),optpath(1:end,3),'c-','LineWidth',1)
        % Minimum Jerk Trajectory
        plot3(waypts(1:end,1),waypts(1:end,2),waypts(1:end,3),'k-','LineWidth',3)
